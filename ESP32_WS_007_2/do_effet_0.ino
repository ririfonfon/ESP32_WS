  ...
  if (_Pix_mod == 3) {
    for (int i = 0 ; i < NUM_LEDS_PER_STRIP ; i++) {
      if (i >= (_Pix_pos) && i <= _Pix_start + (_Pix_pos)) {
        ...
        pix_buffer[1][i] = pix_colorA[1][i] * pix_coef;
    ...
    for (int i = 0 ; i < numberOfLed ; i++) {
      if (i >= (_Pix_pos) && i <= _Pix_start + (_Pix_pos)) {
        ...
        if (i <= N_L_P_S) {
          ...
          pix_buffer[2][i - N_L_P_S * 2] = pix_colorA[2][i - N_L_P_S * 2] * pix_coef;
        } else if (i <= N_L_P_S * 4 & i >= N_L_P_S * 3) {
          pix_buffer[3][i - N_L_P_S * 3] = pix_colorA[3][ - N_L_P_S * 3] * pix_coef;
        }
      } else {
        if (i <= N_L_P_S) {
          pix_buffer[0][i] = pix_colorB[0][i];
  ...
  if (_Pix_mod == 9) {
    for (int i = 0 ; i < NUM_LEDS_PER_STRIP ; i++) {
      if (i >= (_Pix_pos) && i <= _Pix_start + (_Pix_pos)) {
        ...
        pix_buffer[1][i] = ( pix_colorA[1][i] * pix_coef) + (pix_colorB[1][i] * pix_coef_fond);
